  ...
  <div class="recharge-container">
    <div class="recharge-card">
      ...
      <form @submit.prevent="recharge">
        ...
        <div class="form-group">
          <label>金额</label>
          <input type="number" v-model.number="amount" required min="" placeholder="请输入充值金额" />
        </div>
...
async function recharge() {
  message.value = ''
  try {
    const res = await axios.post('http://localhost:3000/card/recharge', {
    ...
    if (res.data && res.data.success) {
      ...
      message.value = res.data.error || '充值失败'
      messageColor.value = 'red'
    }
  } catch (err) {
    message.value = '充值失败：' + (err.response?.data?.error || err.message)
    messageColor.value = 'red'
  }
}
</script>
